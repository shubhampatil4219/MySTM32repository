...
void Worker::DoWork()
{
    ...
    serial->setStopBits(QSerialPort::OneStop);
    serial->setFlowControl(QSerialPort::NoFlowControl);
    serial->setReadBufferSize(16);  // Sets the size of Qt serial port internal read buffer
 //  Note: Do not set the size of qt internal buffer
 // when star, stop bits are used because the program crashes unexpectedly
...
//     rx_array.resize(16);
   ...
   // qDebug()<<rx_array.size();

    // qDebug()<<"Array read before processing"<<rx_array;
   // qDebug()<<"Capacity"<<rx_array.capacity();
   for(int i=0;i<rx_array.size();i++)

    {
        ...
        if(rx_array.at(i)=='s')
        {
           ...
           // for(m=0,n=i+1;(rx_array[n]!='e'||m<rx_array.size());n++,m++)
          //    for(m=0,n=i+1;rx_array[n]!='e';n++,m++)
                 for(m=0,n=i+1;n<rx_array.size()-1;n++,m++)
            {

                correct_data_array[m]=rx_array[n];

            }


            emit send_read_data_to_processing_thread(correct_data_array);
    ...
    //   emit send_read_data_to_processing_thread(rx_array);


    // Sends data to data_processing thread
