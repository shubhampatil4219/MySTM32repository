/*
 * Specific_parameters_main_BAT3.c
 *
...
void Read_and_Compare_Specific_para_Bat3_STM_to_Pi()
{


	uint8_t tx_bat3_specf_battery_status[1] = {0x16};		// 0x16
	...
	HAL_I2C_Master_Transmit_IT(&hi2c3, 0x16,tx_bat3_specf_battery_status, 1);
	HAL_Delay(5);
	...
	uint8_t send_specific_batstatus[16]={'s','3','M','S','I','B','S','S',rx_batstatus[1],rx_batstatus[0],'B','S','S','I','E','e'};
	HAL_UART_Transmit_IT(&huart2, send_specific_batstatus, 16);



	// Battery Full Charge Capacity
	uint8_t tx_bat3_specf_full_charge_capacity[1]={0x10};	// 0x10
	uint8_t rx_full_charge_capacity[2]={0x00,0x00};
	HAL_I2C_Master_Transmit_IT(&hi2c3, 0x16, tx_bat3_specf_full_charge_capacity, 1);
	HAL_Delay(5);
	HAL_I2C_Master_Receive_IT(&hi2c3, 0x16, rx_full_charge_capacity, 2);
	HAL_Delay(1);
...
//	printf("Full Charge Capacity = %d\n",fcc);
	...
	uint8_t send_specific_fcc[16]={'s','3','M','S','I','F','C','S',rx_full_charge_capacity[1],rx_full_charge_capacity[0],'F','C','S','I','E','e'};
	...
	uint8_t tx_bat3_specf_design_capacity[1]={0x18};		// 0x18
	uint8_t rx_design_capacity[2]={0x00,0x00};
	HAL_I2C_Master_Transmit_IT(&hi2c3, 0x16,tx_bat3_specf_design_capacity, 1);
	HAL_Delay(5);
	HAL_I2C_Master_Receive_IT(&hi2c3, 0x16, rx_design_capacity, 2);
	HAL_Delay(1);
...
//	printf("Design Capacity = %d\n",designcap);
	...
	uint8_t send_specific_design_cap[16]={'s','3','M','S','I','D','C','S',rx_design_capacity[1],rx_design_capacity[0],'D','C','S','I','E','e'};
	...
	Compare_Specifi_para_Bat3(fcc,designcap,bs);   // Function to compare specific parameters of battery 1
...
void Compare_Specifi_para_Bat3(uint16_t fullchargecap , uint16_t designcap, uint16_t batterystatus)
{
	...
	{
		Bat3_Maint_Autotestflag = false;   // // Abort the test
...
void Read_bat3_maint_Run_time_to_empty_start_of_discharge()
{

	uint8_t tx_rte[1]={0x11};	// 0x11
	uint8_t rx_rte[2]={0x00,0x00};
	HAL_I2C_Master_Transmit_IT(&hi2c3, 0x16, tx_rte, 1);
	HAL_Delay(5);
	HAL_I2C_Master_Receive_IT(&hi2c3, 0x16, rx_rte, 2);
	HAL_Delay(5);
	uint16_t rte = rx_rte[1];
	rte = rte << 8;
	rte = rte + rx_rte[0];
	uint8_t send_specific_rte[16]={'s','3','M','S','I','R','T','S',rx_rte[1],rx_rte[0],'R','T','F','I','E','e'};
...
void Read_bat3_maint_Run_time_to_empty_end_of_discharge()
{
	...
	rte = rte << 8;
	rte = rte + rx_rte[0];
	uint8_t send_specific_rte[16]={'s','3','M','S','I','R','T','S',rx_rte[1],rx_rte[0],'R','T','L','I','E','e'};
