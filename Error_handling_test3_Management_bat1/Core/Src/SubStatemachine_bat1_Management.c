/*
 * SubStatemachine_bat1_Management.c
 *
...
extern bool BAT_1_MANAGEMENT_FLAG ;  // Flag to enter Bat 1 in Management Use Case

extern bool BAT_1_MANAGEMENT_ENTERED_GUARD_FLAG; // Flag to send instruction only once to Pi that Bat 1
												  // has entered Management Use Case

extern bool BAT_1_MANAGEMENT_GENERAL_INFO_FLAG ; // Flag to

extern bool BAT_1_MANAGEMENT_GENERAL_INFO_GUARD_FLAG;    // Flag to display General Info only once
...
void substatemachine_battery1_management()
{

	if(BAT_1_MANAGEMENT_FLAG==true)
	{
		if(BAT_1_MANAGEMENT_ENTERED_GUARD_FLAG == false)
		{
			BAT_1_MANAGEMENT_ENTERED_GUARD_FLAG = true;
			// Add command to inform Pi that Battery 1 has entered management use case
			// Executed only once
		}


		if(BAT_1_MANAGEMENT_GENERAL_INFO_FLAG == true)
		{

			if(BAT_1_MANAGEMENT_GENERAL_INFO_GUARD_FLAG==false)
			{
				BAT_1_MANAGEMENT_GENERAL_INFO_GUARD_FLAG = true;

				//printf("Displayed General Info once \n");

				Transmit_Bat1_Management_General_Info_to_Raspi(); // This function is working now
				Send_Bat1_Management_Specific_Info_To_Pi();



			}
			else if(BAT_1_MANAGEMENT_GENERAL_INFO_GUARD_FLAG == true)
			{
